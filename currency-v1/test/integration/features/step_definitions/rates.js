...
module.exports = function () {
  this.When(/^I request all exchange rates with default values$/, function (
    callback
  ) {
  ...
  this.When(
    /^I request all exchange rates with (.{3}) as the base currency$/,
  ...
  this.When(/^I request all exchange rates for (.*)$/, function (
    date,
    callback
  ) {
  ...
  this.When(
    /^I request all exchange rates with (.{3}) as the base currency for (.*)$/,
    function (base, date, callback) {
      this.apickli.queryParameters.base = base;
      this.apickli.get('/' + date, callback);
    }
  );

  this.Then(/^I should see (.*) as the base currency$/, function (
    base,
    callback
  ) {
    var self = this;
    assertSuccessfulApiResponse(this.apickli)
      .then(function () {
        var assertion = self.apickli.assertPathInResponseBodyMatchesExpression(
          '$.base',
          base
        );
        if (assertion.success) {
  ...
  this.Then(/^I should see the rates for the latest exchange day$/, function (
    callback
  ) {
    var self = this;
    assertSuccessfulApiResponse(this.apickli)
      .then(function () {
        ...
        if (dateInResponse >= fromDate && dateInResponse <= toDate) {
          ...
          callback(
            'response date ' + dateInResponse + ' is not in acceptance range'
          );
        }
      })
      .catch(function (assertion) {
  ...
  this.Then(/^I should see the rates for (\d{4}-\d{2}-\d{2})$/, function (
    date,
    callback
  ) {
    var self = this;
    console.log('This be the date: ', date);
    assertSuccessfulApiResponse(this.apickli)
      .then(function () {
        var assertion = self.apickli.assertPathInResponseBodyMatchesExpression(
          '$.date',
